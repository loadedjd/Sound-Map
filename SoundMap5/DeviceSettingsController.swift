...
class DeviceSettingsController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate, UITextFieldDelegate {
    ...
    func setupView() {
        ...
        let data = CoredataManager.sharedInstance.settings
        
        if data.count > 0 {
            let setting = data[0]
            
            if let code = setting.code {
                self.databaseField.text = code
            }
            
            if let device = setting.device {
                let index = self.devices.index(of: device)
    ...
    @objc func saveButtonWasPressed() {
        ...
        CoredataManager.sharedInstance.updateSetting(code: self.databaseField.text!, device: self.devices[self.devicePicker.selectedRow(inComponent: 0)])
...
extension DeviceSettingsController {
